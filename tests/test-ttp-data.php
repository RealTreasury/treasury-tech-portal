...
class TTP_Data_Test extends TestCase {
    ...
    protected function setUp(): void {
        ...
        $linked_maps = [
            'Regions'        => [ 'Name'   => 'fld_reg_name' ],
            'Vendors'        => [ 'Name'   => 'fld_vend_name' ],
            'Hosted Type'    => [ 'Name'   => 'fld_host_name' ],
            'Domain'         => [ 'Domain' => 'fld_dom_name' ],
            'Category'       => [ 'Name'   => 'fld_cat_name' ],
            'Sub Categories' => [ 'Name'   => 'fld_sub_name' ],
            'Capabilities'   => [ 'Name'   => 'fld_cap_name' ],
        ];
        \Patchwork\replace('TTP_Airbase::get_table_schema', function ( $table_id = '' ) use ( &$schema, $linked_maps ) {
            return $linked_maps[ $table_id ] ?? $schema;
        });
    }
    ...
    public function test_refresh_vendor_cache_maps_fields() {
        ...
        $fields_used = [];
        $use_ids     = [];
        \Patchwork\replace('TTP_Airbase::resolve_linked_records', function ($table_id, $ids, $primary_field = 'Name', $use_field_ids = false) use (&$tables, &$fields_used, &$use_ids) {
            $tables[]               = $table_id;
            $fields_used[ $table_id ] = $primary_field;
            $use_ids[ $table_id ]     = $use_field_ids;
            $maps = [
                'Regions'        => [
        ...
        $this->assertSame('fld_reg_name', $fields_used['Regions']);
        $this->assertSame('fld_vend_name', $fields_used['Vendors']);
        $this->assertSame('fld_host_name', $fields_used['Hosted Type']);
        $this->assertSame('fld_dom_name', $fields_used['Domain']);
        $this->assertSame('fld_cat_name', $fields_used['Category']);
        $this->assertSame('fld_sub_name', $fields_used['Sub Categories']);
        $this->assertSame('fld_cap_name', $fields_used['Capabilities']);
        foreach ( $use_ids as $flag ) {
