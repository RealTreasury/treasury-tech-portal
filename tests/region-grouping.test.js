...
test('intro video restores to header and TRMS video returns when grouping disabled', () => {
  const html = `\
    <div class="treasury-portal">
      <div class="container">
        <div class="intro-video-target" style="display:none;"></div>
        <div id="noResults"></div>
        <div id="listViewContainer"></div>
        <div class="category-section" data-category="CASH" style="display:block;">
          <div class="category-header"><div class="category-video-target"></div></div>
  ...
  // Disable grouping and ensure intro video restored
  portal.groupByCategory = false;
  portal.handleIntroVideoRegion();

  const introVideo = window.document.querySelector('.intro-video-target');
  const trmsVideo = window.document.querySelector('.category-section[data-category="TRMS"] .category-header .category-video-target');

  assert.strictEqual(introVideo.parentElement, container, 'intro video moved back to original container');
  assert.strictEqual(trmsVideo.style.display, '', 'TRMS category video shown');
});
