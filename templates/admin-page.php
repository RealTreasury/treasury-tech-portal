...
// Template for the Treasury Tech Portal admin page.
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}
...
?>
<div class="wrap">
		<h1><?php esc_html_e( 'Treasury Tools', 'treasury-tech-portal' ); ?></h1>
	...
	<form method="get" style="margin-bottom:10px;">
		<input type="hidden" name="page" value="treasury-tools">
				<label for="ttp-sort"><?php esc_html_e( 'Sort By:', 'treasury-tech-portal' ); ?></label>
		<select name="sort" id="ttp-sort" onchange="this.form.submit()">
						<option value="" <?php selected( $current_sort, '' ); ?>><?php esc_html_e( 'Default', 'treasury-tech-portal' ); ?></option>
						<option value="name" <?php selected( $current_sort, 'name' ); ?>><?php esc_html_e( 'Name', 'treasury-tech-portal' ); ?></option>
						<option value="category" <?php selected( $current_sort, 'category' ); ?>><?php esc_html_e( 'Category', 'treasury-tech-portal' ); ?></option>
		</select>
	</form>
        <?php // phpcs:ignore WordPress.Security.NonceVerification.Missing
		if ( ! empty( $_GET['updated'] ) ) :
			?>
				<div class="updated notice"><p><?php esc_html_e( 'Tool saved.', 'treasury-tech-portal' ); ?></p></div>
	<?php endif; ?>
        <?php // phpcs:ignore WordPress.Security.NonceVerification.Missing
		if ( ! empty( $_GET['deleted'] ) ) :
			?>
				<div class="updated notice"><p><?php esc_html_e( 'Tool deleted.', 'treasury-tech-portal' ); ?></p></div>
	<?php endif; ?>
	...
	<table class="widefat">
		...
		<tbody>
			<?php foreach ( $tools as $i => $tool ) : ?>
				<tr>
					...
					<td>
																								<a href="#" class="edit-tool" data-index="<?php echo esc_attr( $i ); ?>"><?php esc_html_e( 'Edit', 'treasury-tech-portal' ); ?></a> |
																								<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=ttp_delete_tool&index=' . $i ), 'ttp_delete_tool' ) ); ?>" onclick="return confirm('<?php echo esc_js( __( 'Delete this tool?', 'treasury-tech-portal' ) ); ?>');"><?php esc_html_e( 'Delete', 'treasury-tech-portal' ); ?></a>
					</td>
				</tr>
			<?php endforeach; ?>
		</tbody>
		...
		<h2 id="add-new-tool"><?php esc_html_e( 'Add / Edit Tool', 'treasury-tech-portal' ); ?></h2>
		...
		<table class="form-table">
			<tr>
								<th><label for="tool-name"><?php esc_html_e( 'Name', 'treasury-tech-portal' ); ?></label></th>
				<td><input name="name" id="tool-name" type="text" class="regular-text" required></td>
			</tr>
			<tr>
								<th><label for="tool-category"><?php esc_html_e( 'Category', 'treasury-tech-portal' ); ?></label></th>
				<td><input name="category" id="tool-category" type="text" class="regular-text" required></td>
			</tr>
			<tr>
								<th><label for="tool-desc"><?php esc_html_e( 'Description', 'treasury-tech-portal' ); ?></label></th>
				<td><textarea name="desc" id="tool-desc" class="large-text"></textarea></td>
			</tr>
			<tr>
								<th><label for="tool-features"><?php esc_html_e( 'Features (one per line)', 'treasury-tech-portal' ); ?></label></th>
				<td><textarea name="features" id="tool-features" class="large-text"></textarea></td>
			</tr>
			<tr>
								<th><label for="tool-target"><?php esc_html_e( 'Target', 'treasury-tech-portal' ); ?></label></th>
				<td><input name="target" id="tool-target" type="text" class="regular-text"></td>
			</tr>
			<tr>
								<th><label for="tool-video"><?php esc_html_e( 'Video URL', 'treasury-tech-portal' ); ?></label></th>
				<td><input name="videoUrl" id="tool-video" type="url" class="regular-text"></td>
			</tr>
			<tr>
								<th><label for="tool-website"><?php esc_html_e( 'Website URL', 'treasury-tech-portal' ); ?></label></th>
				<td><input name="websiteUrl" id="tool-website" type="url" class="regular-text"></td>
			</tr>
			<tr>
								<th><label for="tool-logo"><?php esc_html_e( 'Logo URL', 'treasury-tech-portal' ); ?></label></th>
